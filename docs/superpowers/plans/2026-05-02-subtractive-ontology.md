...
**Working directory for all `git`/`ls`/path commands in this plan:** `~/test/sounds-and-recreation/reverse-synth-research`. The current branch is `feature/subtractive-ontology-mvp-spec` and the spec + backlog are already committed there.
